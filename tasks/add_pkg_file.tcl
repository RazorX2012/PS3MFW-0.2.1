...
namespace eval add_pkg_file {
    ...
    array set ::add_pkg_file::options {
		...
		--pkg-label3 "                     Files Will Be Added To /dev_flash/PS3Ultimate/Packages                       : :"
    ...
    proc main {} {
		...
		set EXPLORE_PLUGIN_SPRX [file join dev_flash vsh module explore_plugin.sprx]

        ::modify_devflash_file $CATEGORY_GAME_TOOL2_XML ::add_pkg_file::find_nodes
		::modify_devflash_file $CATEGORY_GAME_XML ::add_pkg_file::inject_nodes
		::modify_devflash_file $EXPLORE_PLUGIN_SPRX ::add_pkg_file::patch_sprx
		::modify_devflash_file $CATEGORY_GAME_TOOL2_XML ::add_pkg_file::callback_patch
		::modify_devflash_file $CATEGORY_GAME_XML ::add_pkg_file::callback_patch2
		...
		if {$::add_pkg_file::options(--pkg-enable)} {
			...
			file mkdir [file join ${::CUSTOM_PUP_DIR} dev_flash PS3Ultimate Packages]
		...
		if {[file exists $options(--pkg-src)] == 0} {
            log "Skipping Package 1 does not exist"
		        ...
		        log "Adding $src to /dev_flash/PS3Ultimate/Packages"
		...
		if {[file exists $options(--pkg-src2)] == 0} {
            log "Skipping Package 2 does not exist"
		        ...
		        log "Adding $src2 to /dev_flash/PS3Ultimate/Packages"
				log "Please Install Through Install Package Files"
		...
		if {[file exists $options(--pkg-src3)] == 0} {
            log "Skipping Package 3 does not exist"
		        ...
		        log "Adding $src3 to /dev_flash/PS3Ultimate/Packages"
				log "Please Install Through Install Package Files"
		...
		if {[file exists $options(--pkg-src4)] == 0} {
            log "Skipping Package 4 does not exist"
		        ...
		        log "Adding $src4 to /dev_flash/PS3Ultimate/Packages"
				log "Please Install Through Install Package Files"
		...
		if {[file exists $options(--pkg-src5)] == 0} {
            log "Skipping Package 5 does not exist"
				...
				log "Adding $src5 to /dev_flash/PS3Ultimate/Packages"
				log "Please Install Through Install Package Files"
	...
	proc callback_patch {path args} {	
		...
		set REPLACE3 "/dev_ms"
		set REPLACE4 "/dev_flash/PS3Ultimate/Packages"
		
		sed_in_place [file join $path] $REPLACE1 $REPLACE2
		sed_in_place [file join $path] $REPLACE3 $REPLACE4
	}
	...
	proc patch_sprx {path args} {
		set EXPLORE_PLUGIN_SPRX [file join dev_flash vsh module explore_plugin.sprx]
		set EXPLORE_CATEGORY_GAME_SPRX [file join dev_flash vsh module explore_category_game.sprx]
		
		if {$::add_pkg_file::options(--pkg-enable5)} {		
			::modify_devflash_file $EXPLORE_PLUGIN_SPRX ::add_pkg_file::patch_self
			::modify_devflash_file $EXPLORE_CATEGORY_GAME_SPRX ::add_pkg_file::patch_self
		}
	}
	...
	proc patch_self { self } {	
        if {!$::add_pkg_file::options(--pkg-enable5)} {
            ...
            ::modify_self_file $self ::add_pkg_file::patch_elf
		}
    }
	
	proc patch_elf { elf } {
		if {$::add_pkg_file::options(--pkg-enable5) } {
		    log "Patching [file tail $elf] To Allow Installing Packages"
