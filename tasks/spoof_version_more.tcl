...
namespace eval ::spoof_version_more {
    ...
    proc main {} {
      ...
      if {$release != "" || $build != "" || $target != "" || $auth != ""} {
        ...
        ::modify_devflash_file [file join dev_flash vsh etc version.txt] ::spoof_version_more::version_txt
      }
      if {$build != "" || $auth != ""} {
        log "Patching vsh.self"
        ::modify_devflash_file [file join dev_flash vsh module vsh.self] ::spoof_version_more::patch_self
      }
      if {$build != "" && $bdate != ""} {
        log "Patching UPL.xml"
        ::modify_upl_file ::spoof_version_more::upl_xml
      }
    }

    proc patch_self {self} {
      ::modify_self_file $self ::spoof_version_more::patch_elf
    }
